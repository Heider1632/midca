...
class Memory:
	...
	def clear(self):
		...
		self.locks.clear()
	# in this method i can access to goalgraph, log, etc...
	def get(self, structname):
		# here two form that save the information
		# the first without logAccess
		with self.mainLock:
			if structname not in self.locks:
				return None #if there is knowledge stored there must be a lock.
		# the second within logAcess
		with self.locks[structname]:
			self.logAccess(structname)
			if structname in self.knowledge:
	...
	def logAccess(self, key):
		if self.logger and self.logEachAccess:
			# this function pass to log class with the dates saved in memory
			# but i dont have idea in that structure is passed.
